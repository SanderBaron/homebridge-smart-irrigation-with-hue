...
- **Mid-run schedule abort** — turning off any zone valve in Apple Home while a scheduled (or "Run Schedule Now") programme is in progress now immediately aborts the entire sequence: queue cleared, all running zones closed, pump stopped. Previously the remaining steps would continue firing on their original timers even after manually closing valves. The programme can always be restarted via the "Run Schedule Now" switch or by opening individual valves manually.
